...
use crate::tool::{ShellHandler, ToolRegistry, ToolType, custom, shell, tool_search};
use crate::types::io::{OutputItem, ShellCallAction, ShellCallStatus};
...
impl<'a> FunctionSseTranslator<'a> {
    ...
    fn start_call(
        ...
        match tool_type {
            ...
            ToolType::Shell if !self.registry.is_gateway_owned_name(name) => {
                self.active.insert(
                    output_index,
    ...
    fn unfinished_tool_search_item_ids(&self) -> HashSet<String> {
        let active = self.active.values().filter_map(|shape| match shape {
            ...
            FunctionCallShape::PublicFunction
            | FunctionCallShape::GatewayOwned
            | FunctionCallShape::Custom(_)
            | FunctionCallShape::Shell(_) => None,
        });
        let pending = self
            .registry
...
mod tests {
    ...
    fn shell_function_arguments_restore_openai_shell_lifecycle() {
        let mut accumulator = ResponseAccumulator::new("resp_1".to_owned(), None);
        let registry = test_registry(HashMap::from([("shell".to_owned(), ToolType::Shell)]));
        let mut translator = FunctionSseTranslator::new(&registry);
        let events = [
            serde_json::json!({
                "type": "response.output_item.added", "output_index": 0,
